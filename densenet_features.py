...
class DenseNet_features(nn.Module):
    ...
    def __init__(self, out_channels, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_classes=1000, depth=5):
...
def densenet121_features(pretrained=False, **kwargs):
    ...
    model = DenseNet_features(out_channels, num_init_features=64, growth_rate=32, block_config=(6, 12, 24, 16),
                     **kwargs)
    if pretrained:
...
def densenet169_features(pretrained=False, **kwargs):
    ...
    model = DenseNet_features(out_channels, num_init_features=64, growth_rate=32, block_config=(6, 12, 32, 32),
                     **kwargs)
    if pretrained:
        # '.'s are no longer allowed in module names, but pervious _DenseLayer
        # has keys 'norm.1', 'relu.1', 'conv.1', 'norm.2', 'relu.2', 'conv.2'.
...
def densenet201_features(pretrained=False, **kwargs):
    ...
    model = DenseNet_features(out_channels, num_init_features=64, growth_rate=32, block_config=(6, 12, 48, 32),
                     **kwargs)
    if pretrained:
        # '.'s are no longer allowed in module names, but pervious _DenseLayer
        # has keys 'norm.1', 'relu.1', 'conv.1', 'norm.2', 'relu.2', 'conv.2'.
...
def densenet161_features(pretrained=False, **kwargs):
    ...
    model = DenseNet_features(out_channels, num_init_features=96, growth_rate=48, block_config=(6, 12, 36, 24),
                     **kwargs)
    if pretrained:
        # '.'s are no longer allowed in module names, but pervious _DenseLayer
        # has keys 'norm.1', 'relu.1', 'conv.1', 'norm.2', 'relu.2', 'conv.2'.
