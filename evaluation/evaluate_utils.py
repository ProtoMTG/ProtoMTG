...
from utils.multitask_utils import get_output, mkdir_if_missing
...
class PerformanceMeter(object):
    ...
    def __init__(self, task_kwargs):
        ...
        self.meters = {t: get_single_task_meter(task_kwargs, t) for t in self.tasks}
    ...
    def get_score(self, logger=print, verbose=True):
        eval_dict = {}
        for t in self.tasks:
            eval_dict[t] = self.meters[t].get_score(logger, verbose)
...
def get_single_task_meter(task_kwargs, task):
    """ Retrieve a meter to measure the single-task performance """
    # task_type = task_kwargs['task_type'][task]
    if task == 'semseg':
        from evaluation.eval_semseg import SemsegMeter, MultiClassSemsegMeter
        return SemsegMeter(task_kwargs, task)
    elif task.startswith('semseg'):
        from evaluation.eval_semseg import MultilabelSemsegMeter
        return MultilabelSemsegMeter(task_kwargs, task)

    elif task.startswith('gen'): # Single task performance meter uses the loss (True evaluation is based on seism evaluation)
        from evaluation.eval_generation import GenMeter
        return GenMeter()
    else:
        raise NotImplementedError
