...
class BalancedCrossEntropyLoss(Module):
    ...
    def forward(self, output, label, void_pixels=None):
        ...
        output_gt_zero = torch.ge(output, 0).float() # output_gt_zero可以理解成预测，大于0为1，小于0为0
        # torch.mul(output, (labels - output_gt_zero)): 如果 labels 表示正确的标签，而 output_gt_zero 表示预测的输出大于等于0，那么 (labels - output_gt_zero) 的结果为负值，从而在预测错误时增加损失
