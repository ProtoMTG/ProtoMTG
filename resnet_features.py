...
class ResNet_features(nn.Module):
    ...
    def __init__(self, block, layers, out_channels, depth=5, num_classes=1000, zero_init_residual=False):
...
def resnet18_features(pretrained=False, **kwargs):
    ...
    model = ResNet_features(BasicBlock, [2, 2, 2, 2], out_channels, **kwargs)
    if pretrained:
...
def resnet34_features(pretrained=False, **kwargs):
    ...
    """
    out_channels = (3, 64, 64, 128, 256, 512)
    model = ResNet_features(BasicBlock, [3, 4, 6, 3], out_channels, **kwargs)
    if pretrained:
...
def resnet50_features(pretrained=False, **kwargs):
    ...
    model = ResNet_features(Bottleneck, [3, 4, 6, 3], out_channels, **kwargs)
    if pretrained:
...
def resnet101_features(pretrained=False, **kwargs):
    ...
    """
    out_channels = (3, 64, 256, 512, 1024, 2048)
    model = ResNet_features(Bottleneck, [3, 4, 23, 3], out_channels, **kwargs)
    if pretrained:
...
def resnet152_features(pretrained=False, **kwargs):
    ...
    """
    out_channels = (3, 64, 256, 512, 1024, 2048)
    model = ResNet_features(Bottleneck, [3, 8, 36, 3], out_channels, **kwargs)
    if pretrained:
